...
public class CustomRoutingCalculator {
     ...
     */
    private static final int MAX_TEMPLATE_CLUSTER_SIZE = 5;
    ...
    /*
     * Run omni-directional BFS from source to sink to find a set of compliant templates. Different sinks are favored
     *  over different sources.
     */
    public static ArrayList<RouteTemplate> createRouteTemplateBatch(Design d, EnterWireJunction src,
                                                                    ExitWireJunction snk) {
        ...
        queue.add(new JunctionsTracer(src, 0));

        int templateCount = 0;
        long tBegin = System.currentTimeMillis();
        while (templateCount < MAX_TEMPLATE_CLUSTER_SIZE) {
            ...
            for (ExitWireJunction exit : fanOut) {
                ...
                if (FabricBrowser.globalNodeFootprint.contains(wireDest.getNodeName())
                        || footprint.contains(wireDest.getNodeName()) || CustomRouter.isLocked(wireDest.getNodeName())
                        || FabricBrowser.globalNodeFootprint.contains(exit.getNodeName())
                        || footprint.contains(exit.getNodeName()) || CustomRouter.isLocked(exit.getNodeName()))
                    continue;
        ...
        else if (results.size() < MAX_TEMPLATE_CLUSTER_SIZE)
     ...
     *  over different sources.
     */
    public static ArrayList<ArrayList<RouteTemplate>> createBussedRouteTemplateBatches(Design d,
                                                                                       ArrayList<EnterWireJunction> srcs,
        ...
        ArrayList<ArrayList<RouteTemplate>> results = new ArrayList<>();
        for (int i = 0; i < bitwidth; i++) {
            ArrayList<RouteTemplate> templateChoices = CustomRoutingCalculator.createRouteTemplateBatch(d, srcs.get(i),
                    snks.get(i));
            results.add(templateChoices);
        ...
        for (int i = 0; i < results.size(); i++) {
            ArrayList<RouteTemplate> templateCandidates = results.get(i);
            for (int j = 0; j < templateCandidates.size();) {
                ...
                ArrayList<Integer> conflictedBits = locateTemplateCollisions(nodeUsage, remainingUsages);
                if (!conflictedBits.isEmpty()) {
                    int minTemplateChoices = 99;
                    for (int b : conflictedBits) {
                        if (results.get(b).size() < minTemplateChoices)
                            minTemplateChoices = results.get(b).size();
                    }
                    ...
                    for (int b : conflictedBits) {
                        for (int k = 0; k < results.get(b).size();) {

                            boolean isConflicted = false;
                            for (WireJunction junction : results.get(b).get(k).getTemplate()) {
        ...
        for (ArrayList<RouteTemplate> templateCandidates : results)
            sizes.add(templateCandidates.size());
        RouterLog.log("Remaining template choices: " + sizes + ". A total of " + purgeCount
     ...
     */
    public static ArrayList<TilePath> deriveBestTilePathConfiguration(Design d, ArrayList<HashSet<TilePath>> allPaths) {
        ...
        RouterLog.log("Deadlock detected. No tile paths configuration is possible.", RouterLog.Level.ERROR);
        return null;
    }
    ...
    @Deprecated
    /*
     * Run omni-directional BFS from sink to source to find a single set of hops. Hops wires are aggregated and put into
     *  a RouteTemplate
     */
    public static RouteTemplate createRouteTemplate(Design d, EnterWireJunction src, ExitWireJunction snk) {
        ...
        while (true) {
            ...
            if (distX == 0 && distY == 0) {
                for (ExitWireJunction leadOut : leadOuts) {
                    if (leadOut.equals(head.getJunction())) {
                        ...
                        JunctionsTracer trav = head;
                        while (trav.getParent() != null) {
                            template.pushExitWireJunction(d, (ExitWireJunction) trav.getJunction());
                            trav = trav.getParent();
                        }
            ...
            Set<EnterWireJunction> fanOut = ((ExitWireJunction) head.getJunction()).isSnk()
                    ? FabricBrowser.findReachableEntrances(d, (ExitWireJunction) head.getJunction())
                    : FabricBrowser.getExitFanOut(d, (ExitWireJunction) head.getJunction());
